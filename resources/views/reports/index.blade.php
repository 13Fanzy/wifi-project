                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr x-show="filterBySearch('{{ addslashes($customer->nama) }}')" class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm">
                            @php
                            // Pelanggan bisa bayar jika:
                            // 1. Belum bayar untuk bulan ini
                            // 2. Status aktif
                            // 3. Bulan tagihan >= bulan bergabung
                            $bulanBergabung = \Carbon\Carbon::parse($customer->tanggal_bergabung)->format('Y-m');
                            $canPay = !$customer->sudah_bayar && $customer->status_aktif && $bulan >= $bulanBergabung;
                            $isPastMonth = $bulan < \Carbon\Carbon::now()->format('Y-m');
                                @endphp
                                @if($canPay)
                                <button type="button"
                                    @click="showPaymentModal = true; selectedCustomer = { id: {{ $customer->id }}, nama: '{{ addslashes($customer->nama) }}', paket: {{ $customer->paket_harga }} }"
                                    class="inline-flex items-center px-3 py-1.5 text-xs font-medium rounded-lg text-white {{ $isPastMonth ? 'bg-orange-500 hover:bg-orange-600' : 'bg-green-600 hover:bg-green-700' }} transition-colors">
                                    <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z" />
                                    </svg>
                                    {{ $isPastMonth ? 'Bayar (Telat)' : 'Bayar' }}
                                </button>
                                @elseif($customer->sudah_bayar && $customer->payment)
                                <div class="flex items-center gap-2">
                                    ...
                                    <form action="{{ route('payments.destroy', $customer->payment) }}" method="POST"
                                        onsubmit="return confirm('Batalkan pembayaran {{ addslashes($customer->nama) }}?')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                                            class="inline-flex items-center px-2.5 py-1.5 text-xs font-medium rounded-lg text-red-700 bg-red-100 hover:bg-red-200 transition-colors">
            ...
            <div x-show="filterBySearch('{{ addslashes($customer->nama) }}')" class="p-4 hover:bg-gray-50 transition-colors">
                ...
                <div class="mt-3">
                    ...
                    $isPastMonth = $bulan < \Carbon\Carbon::now()->format('Y-m');
                        ...
                        <button type="button"
                            @click="showPaymentModal = true; selectedCustomer = { id: {{ $customer->id }}, nama: '{{ addslashes($customer->nama) }}', paket: {{ $customer->paket_harga }} }"
                            class="w-full inline-flex items-center justify-center px-4 py-2 text-sm font-medium rounded-lg text-white {{ $isPastMonth ? 'bg-orange-500 hover:bg-orange-600' : 'bg-green-600 hover:bg-green-700' }} transition-colors">
                            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z" />
                            </svg>
                            {{ $isPastMonth ? 'Bayar Telat' : 'Bayar Sekarang' }}
                        </button>
                        @elseif($customer->sudah_bayar && $customer->payment)
                        <div class="flex gap-2">
                            <a href="{{ route('customers.show', $customer) }}"
                                class="flex-1 inline-flex items-center justify-center px-4 py-2 text-sm font-medium rounded-lg text-indigo-700 bg-indigo-100 hover:bg-indigo-200 transition-colors">
                                Lihat Detail
                            </a>
                            <form action="{{ route('payments.destroy', $customer->payment) }}" method="POST" class="flex-1"
                                onsubmit="return confirm('Batalkan pembayaran {{ addslashes($customer->nama) }}?')">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
                                    class="w-full inline-flex items-center justify-center px-4 py-2 text-sm font-medium rounded-lg text-red-700 bg-red-100 hover:bg-red-200 transition-colors">
                                    <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                                    </svg>
                                    Batalkan
